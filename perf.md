...
Emscripten pthread で色ごとのパイプライン (remove/build/concat) を並列実行。`PTHREAD_POOL_SIZE=navigator.hardwareConcurrency` でコア数に応じたプール。

### 計測結果

| 並列化対象              |    Median | 備考                                                                  |
| ----------------------- | --------: | --------------------------------------------------------------------- |
| なし (シングルスレッド) |     246ms | ベースライン                                                          |
| Pipeline のみ           | **218ms** | 色ごとに独立、自然に並列化                                            |
| Pass1 (色分類) 追加     |     238ms | スレッドローカルHTの確保+マージがオーバーヘッド。**悪化のため不採用** |

### フェーズ内訳と並列化適性

| フェーズ              |   時間 | 並列化                      | 結果                                  |
| --------------------- | -----: | --------------------------- | ------------------------------------- |
| Pass1: 色分類         | ~100ms | スレッドローカルHT + マージ | オーバーヘッド > 改善。メモリ帯域律速 |
| Pass2: エッジ書き込み | ~105ms | 未実施                      | Pass1 同様メモリ帯域律速の可能性高    |
| Pipeline              |  ~34ms | **採用**                    | 34ms → ~9ms (推定)                    |

### 分析

- Pipeline 並列化のみで 246ms → 218ms (**11% 改善**)
- Pass1+Pass2 はメモリ帯域律速。1.25M ピクセル × 4バイトの走査 + ハッシュ検索が支配的で、スレッド分割のオーバーヘッド (HT 確保/マージ) が改善を打ち消す
- **全体 23,431ms → 218ms: ベースラインから 107x 高速化**

---

## 1パス化: Pass1+Pass2 統合

2 パス (色カウント → エッジ書き込み) を 1 パスに統合。色分類と同時にエッジバッファを動的確保 (1.5 倍成長 realloc)。idx_table ハッシュテーブルが不要に。

性能面: input.png で誤差程度の微改善 (217ms→206ms)、dithered 64 色画像で 165ms→157ms。realloc のオーバーヘッドが走査削減を相殺しており大幅な改善には至らないが、劣化もなし。コードの簡潔さ (走査 1 回、ハッシュテーブル 1 つ) を優先して採用。

realloc 回数の削減 (初期容量の推定等) で追加改善の余地はあるが、数 ms レベルの見込みで複雑さに見合わない。
